...
def get_tokenizer(name=DEFAULT_MODEL_NAME):
    tokenizer = T5Tokenizer.from_pretrained(name, model_max_length=MAX_LENGTH)
    
    return tokenizer

def get_model(name=DEFAULT_MODEL_NAME):
    model = T5EncoderModel.from_pretrained(name)
    
    return model

def tokenize(text: List[str], name=DEFAULT_MODEL_NAME):
...
def encode_tokens(token_ids, attn_mask=None, pad_id=0, name=DEFAULT_MODEL_NAME):
...
def encode_text(texts: List[str], name=DEFAULT_MODEL_NAME):
