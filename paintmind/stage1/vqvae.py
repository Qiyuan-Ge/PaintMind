...
class VQVAE(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = Encoder(config.image_size, config.patch_size, config.dim, config.depth, config.heads, config.mlp_dim, config.channels, config.dim_head, config.dropout, config.emb_dropout)
        self.decoder = Decoder(config.image_size, config.patch_size, config.dim, config.depth, config.heads, config.mlp_dim, config.channels, config.dim_head, config.dropout, config.emb_dropout)
    ...
    def forward(self, img):
        latent, commit_loss, indices = self.encode(img)
