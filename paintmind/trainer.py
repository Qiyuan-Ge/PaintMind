...
class PaintMindTrainer:
    ...
    def sample(self, imgs, pred, mask, n=3):
        unwrap_model = self.accelerator.unwrap_model(self.model)
        
        imgs_masked = unwrap_model.patchify(imgs) * (1 - mask).unsqueeze(-1)
        
        imgs = imgs[:n]
        imgs_masked = imgs_masked[:n]
        pred = pred[:n]
        
        pred = unwrap_model.unpatchify(pred)
        imgs_masked = unwrap_model.unpatchify(imgs_masked)
        gen_images = torch.cat([imgs, imgs_masked, pred], dim=0)
        ...
        for epoch in range(self.num_epoch):
            ...
            with tqdm(self.dataloader, dynamic_ncols=True, disable=not self.accelerator.is_local_main_process) as tqdm_dataloader:
                for batch in tqdm_dataloader:
                    ...
                    loss, pred, mask = self.model(imgs, text_emb, text_mask, mask_ratio=0.75)
