...
def distill_loss(students_logits, teacher_logits):
    loss = F.smooth_l1_loss(students_logits, teacher_logits)
    return loss
...
class VQGANTrainer(nn.Module):
    def __init__(
        ...
        self.teacher_model = CLIPImageEmbedder(precision=precision)
                ...
                for batch in train_dl:
                    ...
                    with self.accelerator.accumulate(self.vqvae):
                        with self.accelerator.autocast():
                            rec, loss, cls_feat = self.vqvae(img)
                            rec.detach_()
                            dis_loss = self.dis_loss(cls_feat, self.teacher_model(img))
                            gen_loss = self.gen_loss(self.discr(rec))
                            
                            loss = loss + dis_loss + gen_loss   
                    ...
                    log.update({'vae loss':loss.item(), 'dis loss':dis_loss.item(), 'gen loss':gen_loss.item(), 'discr loss':discr_loss.item(), 'lr':self.optim.param_groups[0]['lr']})
   
                    train_dl.set_postfix(
                        ordered_dict={
                            "epoch"      : epoch,
                            "vae loss"   : log['vae loss'],
                            "dis loss"   : log['dis loss'],
                            "gen loss"   : log['gen loss'],
                            "discr loss" : log['discr loss'],
                            "LR"         : log['lr'],
                        }
                    )
                    self.accelerator.log({"vae loss":log['vae loss'], "dis loss":log['dis loss'], "gen loss":log['gen loss'], "discr loss":log['discr loss'], "lr": log['lr']}, step=self.steps)
